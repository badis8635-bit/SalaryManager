...
const statusStyle = (s) => ({
  ...
  ...(s === 'PAID'      ? { background: 'rgba(71,255,156,0.12)', color: 'var(--success)', border: '1px solid rgba(71,255,156,0.3)' } :
      s === 'PENDING'   ? { background: 'rgba(232,255,71,0.10)', color: 'var(--accent)',  border: '1px solid rgba(232,255,71,0.3)' } :
...
export default function Payslips() {
  ...
  function handleSelect(e) {
    ...
    if (e.target.value) loadPayslips(e.target.value)
  }
  ...
  return (
    <div className="fade-up">
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '32px' }}>
        ...
        {selectedId && <button onClick={() => setShowGen(!showGen)} style={{ padding: '10px 20px', background: 'var(--accent)', border: 'none', borderRadius: '6px', color: '#0f0f0f', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '13px', cursor: 'pointer' }}>
          ...
          {payslips.sort((a,b) => b.period_year - a.period_year || b.period_month - a.period_month).map(p => (
            <div key={p.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                {p.status === 'PENDING' && <>
                  <button onClick={() => handlePay(p.id)} style={{ padding: '5px 12px', background: 'rgba(71,255,156,0.1)', border: '1px solid rgba(71,255,156,0.3)', borderRadius: '4px', color: 'var(--success)', cursor: 'pointer', fontSize: '11px', fontFamily: 'DM Mono, monospace' }}>Payer</button>
                  <button onClick={() => handleCancel(p.id)} style={{ padding: '5px 12px', background: 'transparent', border: '1px solid rgba(255,71,71,0.3)', borderRadius: '4px', color: 'var(--danger)', cursor: 'pointer', fontSize: '11px', fontFamily: 'DM Mono, monospace' }}>Annuler</button>
                </>}
              </div>
            </div>
          ))}
